...
import { App, Button, Card, DatePicker, Select, Spin, Table, Tag } from 'antd'
...
export function AuditPage() {
  ...
  const loadUserOptions = useCallback(async (search: string) => {
    ...
    try {
      ...
      setUserOptions(
        res.data.map((u: EmployeeUser) => ({
          value: u.id,
          label: `${u.employeeId?.trim() || '—'} — ${u.lastName} ${u.firstName}`.trim(),
        })),
      )
    } catch (e) {
      message.error(
  ...
  useEffect(() => {
    void loadUserOptions(debouncedUserSearch)
  }, [debouncedUserSearch, loadUserOptions])
  ...
  useEffect(() => {
    setPage(1)
  }, [actionFilter, userIdFilter, range])
  ...
  const columns: ColumnsType<AuditLog> = useMemo(
    () => [
      ...
      },
      {
        ...
        ellipsis: true,
        render: (_, row) => (
          <span className="audit-col-detail">{formatAuditDetail(row)}</span>
        ),
      },
      {
        title: 'Utilisateur',
